...
% un rimbalzo
us_d =[ -0.1959; 0.2057; 0.2056; -0.1960]*fl;
...
while h<=3
    ...
    if mode==1
        ...
        [~, state, detectedPoint, obj_detected ,~ ]=vrep.simxReadProximitySensor(clientID, h_PS, vrep.simx_opmode_streaming);
        
        if mod(time,5)==0
            disp(obj_detected); % TO-DO return name of object seen
        end
        ...
        for k=1:4
            ...
            % pixel coordinates on image plane
            zs(k)= l_position(3); 
        ...
        L_interaction = [ build_point_jacobian(us(1),vs(1),zs(1),fl); ...
...
%         end
        ...
        %
        err= err - L_interaction*Compliance*(force_torque_d-force_torque);
        %
        ...
        ee_displacement = K*pinv(L_interaction)*err;
...
function [L] = build_point_jacobian(u,v,z,fl)
% THIS IS THE INTERACTION MATRIX [L]

L = [ -fl/z     0          u/z     (u*v)/fl        -(fl+(u^2)/fl)      v; ...
